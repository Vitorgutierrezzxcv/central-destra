...
import { CheckCircle2, Circle, ChevronDown, ChevronUp, Search, Filter, TrendingUp, ShoppingCart, DollarSign, RefreshCw, Star, AlertCircle, Zap } from "lucide-react";

const ALAVANCAS = {
  trafego: { label: "Tráfego", color: "blue", icon: TrendingUp, bg: "bg-blue-50", border: "border-blue-200", badge: "bg-blue-100 text-blue-700" },
  conversao: { label: "Conversão", color: "green", icon: ShoppingCart, bg: "bg-green-50", border: "border-green-200", badge: "bg-green-100 text-green-700" },
  ticket: { label: "Ticket Médio", color: "amber", icon: DollarSign, bg: "bg-amber-50", border: "border-amber-200", badge: "bg-amber-100 text-amber-700" },
  retencao: { label: "Retenção", color: "purple", icon: RefreshCw, bg: "bg-purple-50", border: "border-purple-200", badge: "bg-purple-100 text-purple-700" },
};
...
const CHECKLIST_DATA = [
  ...
  // TICKET MÉDIO - Mensagens no Carrinho
...
export default function ClientPortalEcommerceChecklist() {
  ...
  return (
    <div className="min-h-screen bg-[#f8f8f6] pb-20">
      ...
      <div className="bg-white border-b border-slate-100 px-4 py-6 md:px-8">
        <div className="max-w-4xl mx-auto">
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <h1 className="text-2xl font-bold text-slate-900">Checklist E-commerce</h1>
              <p className="text-sm text-slate-500 mt-1">Guia completo de ações para escalar sua loja virtual</p>
            </div>
            <div className="flex items-center gap-3 bg-slate-50 rounded-2xl px-4 py-3">
              <div className="text-right">
                <p className="text-2xl font-bold text-slate-900">{pct}%</p>
                <p className="text-xs text-slate-500">{totalDone}/{totalAll} concluídos</p>
              </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-5">
            {Object.entries(ALAVANCAS).map(([key, a]) => {
              ...
              return (
                <button
                  key={key}
                  onClick={() => setFilterAlavanca(filterAlavanca === key ? "all" : key)}
                  className={`rounded-xl p-3 text-left transition-all border-2 ${filterAlavanca === key ? `${a.bg} ${a.border}` : "bg-white border-slate-100 hover:border-slate-200"}`}
                >
                  ...
                  <div className="w-full bg-slate-100 rounded-full h-1.5 mb-1">
                    <div className="bg-slate-800 h-1.5 rounded-full transition-all" style={{ width: `${p}%` }} />
                  </div>
                  <p className="text-[10px] text-slate-500">{c.done}/{c.total} feitos</p>
                </button>
              );
            })}
          </div>
      ...
      {/* Filters */}
      <div className="sticky top-0 z-10 bg-white/95 backdrop-blur border-b border-slate-100 px-4 py-3 md:px-8">
        <div className="max-w-4xl mx-auto flex flex-wrap gap-2 items-center">
          <div className="relative flex-1 min-w-[180px]">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
            <input
              className="w-full pl-9 pr-3 py-2 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-300"
          ...
          {(filterAlavanca !== "all" || filterImpacto !== "all" || filterStatus !== "all" || search) && (
            <button
              onClick={() => { setFilterAlavanca("all"); setFilterImpacto("all"); setFilterStatus("all"); setSearch(""); }}
              className="text-xs text-slate-500 hover:text-slate-800 px-2 py-2 underline"
            >
              Limpar filtros
            </button>
          )}
      ...
      <div className="max-w-4xl mx-auto px-4 md:px-8 py-6 space-y-6">
        {filtered.length === 0 && (
          <div className="text-center py-16 text-slate-400">
            ...
            <p className="text-sm">Nenhuma ação encontrada com esses filtros.</p>
          </div>
        )}
        {grouped.map(group => {
          ...
          return (
            <div key={`${group.alavanca}__${group.categoria}`} className="bg-white rounded-2xl border border-slate-100 overflow-hidden">
              <div className={`px-4 py-3 flex items-center justify-between ${aConfig.bg}`}>
                <div className="flex items-center gap-2">
                  <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${aConfig.badge}`}>{aConfig.label}</span>
                  <span className="text-sm font-semibold text-slate-700">{group.categoria}</span>
                </div>
                <span className="text-xs text-slate-500">{doneCnt}/{group.items.length}</span>
              </div>
              <div className="divide-y divide-slate-50">
                {group.items.map(item => {
                  ...
                  return (
                    <div key={item.id} className={`transition-colors ${isDone ? "bg-slate-50/50" : "bg-white"}`}>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between gap-2 flex-wrap">
                            ...
                            <div className="flex items-center gap-1.5 flex-shrink-0">
                              <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${IMPACTO_COLORS[item.impacto]}`}>
                                {item.impacto}
                              </span>
                              <button
                            ...
                            <div className="mt-3 space-y-2 text-sm">
                              ...
                              {item.ferramentas && (
                                <div className="flex items-start gap-2 bg-slate-50 rounded-xl p-3">
                                  ...
                                  <div>
                                    <p className="text-xs font-semibold text-slate-500 mb-0.5">Ferramentas sugeridas</p>
                                    <p className="text-xs text-slate-600">{item.ferramentas}</p>
                                  </div>
                                </div>
                              )}
                            </div>
