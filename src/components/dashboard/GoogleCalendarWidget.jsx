...
export default function GoogleCalendarWidget() {
  ...
  const { data: calendarData, isLoading, refetch } = useQuery({
    ...
    queryFn: () => base44.functions.invoke("getUserCalendarEvents", {
      year: currentMonth.getFullYear(),
      month: currentMonth.getMonth(),
    }).then(r => r?.data ?? r ?? { connected: false, events: [] }),
    staleTime: 0,
    ...
    <div className="bg-white border border-[#EAEAEA] rounded-2xl p-5">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <h2 className="text-sm font-normal text-[#131A20]">
            {connected ? format(currentMonth, "MMMM yyyy", { locale: ptBR }) : "Google Calendar"}
          </h2>
          {connected && (
            <span className="text-[10px] bg-green-100 text-green-600 px-1.5 py-0.5 rounded-full font-light">conectado</span>
          )}
        </div>
        {connected && (
      ...
      {!isLoading && !connected && (
        <div className="py-6 flex flex-col items-center gap-4 text-center">
          <div className="w-12 h-12 bg-[#EBF3FF] rounded-2xl flex items-center justify-center">
            ...
            {days.map(day => {
              ...
              return (
                <button key={day.toISOString()} onClick={() => setSelectedDate(day)}
                  ...
                  {format(day, "d")}
                  {dayEvents.length > 0 && (
                    <span className="absolute bottom-0.5 w-1 h-1 rounded-full bg-[#6FA6FF]" />
                  )}
                </button>
              );
            })}
